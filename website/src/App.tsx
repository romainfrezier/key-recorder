...
function Header({ locale }: { locale: Locale }) {
  ...
  return <header className="site-header">
    <Link className="brand" to={`/${locale}/`} aria-label="Key Recorder home"><span className="brand-mark" aria-hidden="true">KR</span><span>Key Recorder</span></Link>
...
function Footer({ locale }: { locale: Locale }) {
  const { t } = useTranslation()
  return <footer className="site-footer"><div><div className="footer-brand"><span className="brand-mark" aria-hidden="true">KR</span> Key Recorder</div><p>{t('footer.tagline')}</p></div><div className="footer-links"><Link to={`/${locale}/researchers/`}>{t('nav.researchers')}</Link><Link to={`/${locale}/privacy/`}>{t('nav.privacy')}</Link><a href="https://github.com/romainfrezier/key-recorder" target="_blank" rel="noreferrer">GitHub ↗</a></div><div className="footer-note">{t('footer.license')}</div></footer>
}

function ProtocolStrip() {
  const { t } = useTranslation()
  return <div className="protocol-card" aria-label={t('home.protocol.aria')}><div className="protocol-topline"><span>{t('home.protocol.label')}</span><span className="status-dot">{t('home.protocol.status')}</span></div><div className="protocol-keys"><div className="protocol-key key-coral"><kbd>A</kbd><span>{t('home.protocol.keyA')}</span></div><div className="protocol-operator">+</div><div className="protocol-key key-blue"><kbd>B</kbd><span>{t('home.protocol.keyB')}</span></div></div><div className="protocol-data"><span><b>10:00</b> {t('home.protocol.duration')}</span><span><b>00:30</b> {t('home.protocol.interval')}</span><span className="protocol-live">{t('home.protocol.recording')}</span></div><div className="protocol-csv"><span>interval</span><span>{t('home.protocol.keyA')}</span><span>{t('home.protocol.keyB')}</span><span>0s–30s</span><span>12.450</span><span>4.200</span></div></div>
}
...
function HomePage({ locale }: { locale: Locale }) {
  ...
  return <>
    <section className="hero section-shell"><div className="hero-copy"><p className="eyebrow"><span className="eyebrow-line" />{t('home.eyebrow')}</p><h1>{t('home.hero.title')} <em>{t('home.hero.emphasis')}</em></h1><p className="hero-intro">{t('home.hero.body')}</p><div className="hero-actions"><a className="button button-primary" href={releaseUrl} target="_blank" rel="noreferrer">{t('home.hero.cta')} <span>↗</span></a><Link className="text-link" to={`/${locale}/researchers/`}>{t('home.hero.secondary')} <span>↓</span></Link></div><p className="micro-note"><span className="apple-dot">●</span>{t('home.hero.requirement')}</p></div><div className="hero-visual"><div className="registration-mark mark-top">01 — MEASURE</div><ProtocolStrip /><div className="registration-mark mark-bottom">FIELD NOTE / 001</div></div></section>
...
function LanguageLanding() {
  ...
  return <main className="language-landing"><div className="landing-card"><span className="brand-mark" aria-hidden="true">KR</span><p className="eyebrow">KEY RECORDER / SELECT LANGUAGE</p><h1>{t('landing.title')}</h1><div className="landing-options">{locales.map((locale) => <button key={locale} type="button" onClick={() => navigate(`/${locale}/`)}><span>{locale === 'en' ? 'English' : locale === 'fr' ? 'Français' : 'Italiano'}</span><span>↗</span></button>)}</div></div></main>
}
